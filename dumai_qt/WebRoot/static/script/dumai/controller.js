...
define(['jquery',"angular","angular-ui-router","ng-grid",'angular-animate'],function ($,angular) {
    ...
    ruleCtrlMod.controller("myUser", ['$scope','$rootScope','$stateParams','$http',function ($scope,$rootScope,$stateParams,$http) {
        ...
        $http.get('static/data/ruleList'+aa+'.json').success(function (data) {
       ...
       // 大put框中点击保存按钮的事件
        $scope.largeRuleAdd=function () {
        	$firstInput=$('#gridStyle table thead input');
            $oInput=$('#gridStyle table tbody input');
