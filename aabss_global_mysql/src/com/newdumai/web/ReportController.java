...
public class ReportController {
    ...
    public JsonResult report(HttpServletRequest request, HttpServletResponse response) {
        ...
        String biz_range = request.getParameter("biz_range");
        ...
        //电核
        data.put("dhData", auditingTaskService.getDhResultByOrderCode(code));

        //详细数据列表
        data.put("detailInterfaces", reportService.getDetailInterfaces(data));

        //订单分期情况
        if ("2".equals(biz_range)) {
            Map<String, Object> orderBillMap = reportService.getOrderBill(code);
            data.put("orderBill", orderBillMap);
        }
    ...
    @RequestMapping(value = "/report/report2.do", method = RequestMethod.GET)
    @ResponseBody
    public JsonResult report2(HttpServletRequest request, HttpServletResponse response) {
        String code = request.getParameter("code");
        List<Map<String, Object>> list = reportService.getDetailInterfaceMap(code);
     ...
     * @return 多个报告详细数据(顺序与请求参数report[]指定的一致)
     */
    @RequestMapping(value = "/report/dataDetail2.do", method = {RequestMethod.POST, RequestMethod.GET})
    @ResponseBody
    public List<Map<String, Object>> dataDetail2(HttpServletRequest request, HttpServletResponse response) {
...
//				return JsonResult.successResult(genReportService.dataDetailRaw(dm_3rd_interface_code,orderCode));
                ...
                return genReportService.dataDetail2(dm_3rd_interface_code, orderCode);
            }
        } catch (Exception e) {
            e.printStackTrace();
            return null;
